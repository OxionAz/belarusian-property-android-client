...
import com.oxionaz.belarussian_property.view.activities.RentFlatsSearchActivity_;
...
public class RentFlatsFragment extends BasePropertyFragment {
    ...
    @Inject
    RentFlatsPresenter rentFlatsPresenter;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        rentFlatsPresenter.onCreate(this);
        mainActivityView = (MainActivityView) getActivity();
    }

    @AfterViews
    void ready(){
        getActivity().setTitle("Квартиры");
        ...
        propertyAdapter = new PropertyAdapter(new PropertyAdapter.PropertyViewHolder.ClickListener() {
            ...
            public void OnFavoriteButtonClicked(Integer favorite, int position) {
                if (favorite != null) {
                    rentFlatsPresenter.deleteFavorite(propertyAdapter.getFavoriteByPosition(favorite));
                    propertyAdapter.deleteFavorite(favorite);
                } else {
                    rentFlatsPresenter.saveFavorite(propertyAdapter.getItemByPosition(position));
                ...
                if (dy > 0 || dy < 0)
                {
                    ...
                    if (loadable)
                    {
                        if ( (visibleItemCount + pastVisiblesItems) >= totalItemCount)
                        {
                            ...
                            rentFlatsPresenter.nextLoad(true);
                        }
                    }
                }
            }
        ...
        swipe_refresh.setOnRefreshListener(() -> rentFlatsPresenter.updateList(false));
        rentFlatsPresenter.firstLoad(true);
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if (preferencesHelper.getFragmentPreference("rent_flats", "date_asc")) {
            option_sort_date_asc.setChecked(true);
        } else if (preferencesHelper.getFragmentPreference("rent_flats", "date_desc")) {
            option_sort_date_desc.setChecked(true);
        } else if (preferencesHelper.getFragmentPreference("rent_flats", "price_asc")) {
            option_sort_price_asc.setChecked(true);
        } else if (preferencesHelper.getFragmentPreference("rent_flats", "price_desc")) {
    ...
    public void onResume() {
        super.onResume();
        rentFlatsPresenter.getFavoritesFromCache();
    }
    ...
    void sort_def() {
        option_sort_default.setChecked(true);
        preferencesHelper.setFragmentPreference("rent_flats", "date_asc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "date_desc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_asc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_desc", false);
        rentFlatsPresenter.updateList(true);
    }

    @OptionsItem(R.id.option_sort_date_asc)
    void sort_date_asc() {
        option_sort_date_asc.setChecked(true);
        preferencesHelper.setFragmentPreference("rent_flats", "date_asc", true);
        preferencesHelper.setFragmentPreference("rent_flats", "date_desc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_asc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_desc", false);
        rentFlatsPresenter.updateList(true);
    ...
    void sort_date_desc() {
        option_sort_date_desc.setChecked(true);
        preferencesHelper.setFragmentPreference("rent_flats", "date_asc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "date_desc", true);
        preferencesHelper.setFragmentPreference("rent_flats", "price_asc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_desc", false);
        rentFlatsPresenter.updateList(true);
    }
    ...
    void sort_price_asc() {
        ...
        preferencesHelper.setFragmentPreference("rent_flats", "date_asc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "date_desc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_asc", true);
        preferencesHelper.setFragmentPreference("rent_flats", "price_desc", false);
        rentFlatsPresenter.updateList(true);
    }
    ...
    void sort_price_desc() {
        ...
        preferencesHelper.setFragmentPreference("rent_flats", "date_desc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_asc", false);
        preferencesHelper.setFragmentPreference("rent_flats", "price_desc", true);
        rentFlatsPresenter.updateList(true);
    }
    ...
    public void showPropertyInfo(PropertyItem item, boolean favorite) {
        Intent intent = new Intent(getContext(), RentFlatActivity_.class);
    ...
    public void startPropertySearch() {
        Intent intent = new Intent(getContext(), RentFlatsSearchActivity_.class);
