...
import com.oxionaz.belarussian_property.view.activities.SaleAreasSearchActivity_;
...
public class SaleAreasFragment extends BasePropertyFragment {
    ...
    @Inject
    SaleAreasPresenter saleAreasPresenter;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        saleAreasPresenter.onCreate(this);
        mainActivityView = (MainActivityView) getActivity();
    }

    @AfterViews
    void ready(){
        getActivity().setTitle("Участки");
        ...
        propertyAdapter = new PropertyAdapter(new PropertyAdapter.PropertyViewHolder.ClickListener() {
            ...
            public void OnFavoriteButtonClicked(Integer favorite, int position) {
                if (favorite != null) {
                    saleAreasPresenter.deleteFavorite(propertyAdapter.getFavoriteByPosition(favorite));
                    propertyAdapter.deleteFavorite(favorite);
                } else {
                    saleAreasPresenter.saveFavorite(propertyAdapter.getItemByPosition(position));
                ...
                if (dy > 0 || dy < 0)
                {
                    ...
                    if (loadable)
                    {
                        if ( (visibleItemCount + pastVisiblesItems) >= totalItemCount)
                        {
                            ...
                            saleAreasPresenter.nextLoad(true);
                        }
                    }
                }
            }
        ...
        swipe_refresh.setOnRefreshListener(() -> saleAreasPresenter.updateList(false));
        saleAreasPresenter.firstLoad(true);
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if (preferencesHelper.getFragmentPreference("sale_areas", "date_asc")) {
            option_sort_date_asc.setChecked(true);
        } else if (preferencesHelper.getFragmentPreference("sale_areas", "date_desc")) {
            option_sort_date_desc.setChecked(true);
        } else if (preferencesHelper.getFragmentPreference("sale_areas", "price_asc")) {
            option_sort_price_asc.setChecked(true);
        } else if (preferencesHelper.getFragmentPreference("sale_areas", "price_desc")) {
    ...
    public void onResume() {
        super.onResume();
        saleAreasPresenter.getFavoritesFromCache();
    }
    ...
    void sort_def() {
        option_sort_default.setChecked(true);
        preferencesHelper.setFragmentPreference("sale_areas", "date_asc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "date_desc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_asc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_desc", false);
        saleAreasPresenter.updateList(true);
    }

    @OptionsItem(R.id.option_sort_date_asc)
    void sort_date_asc() {
        option_sort_date_asc.setChecked(true);
        preferencesHelper.setFragmentPreference("sale_areas", "date_asc", true);
        preferencesHelper.setFragmentPreference("sale_areas", "date_desc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_asc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_desc", false);
        saleAreasPresenter.updateList(true);
    ...
    void sort_date_desc() {
        option_sort_date_desc.setChecked(true);
        preferencesHelper.setFragmentPreference("sale_areas", "date_asc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "date_desc", true);
        preferencesHelper.setFragmentPreference("sale_areas", "price_asc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_desc", false);
        saleAreasPresenter.updateList(true);
    }
    ...
    void sort_price_asc() {
        ...
        preferencesHelper.setFragmentPreference("sale_areas", "date_asc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "date_desc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_asc", true);
        preferencesHelper.setFragmentPreference("sale_areas", "price_desc", false);
        saleAreasPresenter.updateList(true);
    }
    ...
    void sort_price_desc() {
        ...
        preferencesHelper.setFragmentPreference("sale_areas", "date_desc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_asc", false);
        preferencesHelper.setFragmentPreference("sale_areas", "price_desc", true);
        saleAreasPresenter.updateList(true);
    }
    ...
    public void showPropertyInfo(PropertyItem item, boolean favorite) {
        Intent intent = new Intent(getContext(), SaleAreaActivity_.class);
    ...
    public void startPropertySearch() {
        Intent intent = new Intent(getContext(), SaleAreasSearchActivity_.class);
