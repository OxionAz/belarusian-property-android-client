...
public class RentFlatPresenter extends BasePropertyPresenter {
    ...
    public void downloadPropertyInfo(Integer id, Integer section, String price) {
        view.showLoading();
        Subscription subscription = dataManager.downloadRentFlat(Const.API_KEY, id, section, price).map(info -> {
            ...
            return info;
                ...
                .subscribe(new Subscriber<RentFlatDTO>() {
                    @Override
                    ...
                    @Override
                    public void onNext(RentFlatDTO rentFlatDTO) {
                        view.showPropertyInfo(rentFlatDTO);
                    }
                });
    ...
    public void deleteFavorite(PropertyItem item) {
        dataManager.clearFavorite("rent_flats", item.getId(), item.getSection(), item.getPrice());
    }
}
